...
def get_result_config(
    default_config: Dict[str, str], config_path: str
    ...
    try:
        ...
        return {
            **default_config,
            **result_config,
        }  # Что бы не трогать дефолтный конфиг!
    except ValueError:
        return None
...
def log_statistic_calc(
    latest_log: Dict, result_config: Dict, logger: logging.Logger
) -> Optional[List[Dict]]:
    ...
    for url in url_time_list:
        ...
        url_stat_temp["time_perc"] = round(
            url_stat_temp["time_sum"] / time_sum_all_req * 100, 3
        )
...
def html_report_writer(
    result_config: str,
    report_path: str,
    logs_statistic: List[Dict],
    logger: logging.Logger,
) -> bool:
    """
    ...
    with open(report_path, "w", encoding="utf-8") as report, open(
        ...
        if report_path is not None:
            ...
            try:
                ...
                if logs_statistic is None:
                    ...
                    html_report_writer(
                        result_config, report_path, logs_statistic, logger
                    )
